...
9. **The backup key mode never renders words, and that is what makes it safe to offer.** The app
   ...
   **`k` is the dice and nothing else.** Mixing in a generated value by XOR was considered and
   dropped. It would hedge against a biased or observed die, at the cost of making `k` impossible
   for anyone to recompute, including its owner, so nobody could confirm the tool used the dice
   they rolled. That is the one property this project does not trade, and bias is handled the way
   the seed handles it: roll past the minimum, where 60 rolls clear 128 bits even on the
   conservative measure. If XOR is ever revisited it needs this paragraph answered first.

   The honest limit, which the copy states and must keep stating: `k` wraps the backup, and the
   file key inside the age format is generated by the consuming tool and is what the payload is
   actually encrypted under. This is not a claim that no generator is involved anywhere.
